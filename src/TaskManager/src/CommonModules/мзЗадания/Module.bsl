...
// { Работа с заданием
Функция ДобавитьЗадание(Знач Метод, Знач Параметры, Знач Период = Неопределено) Экспорт
	Задание = ПодготовитьМенеджераЗаписиЗадания(Метод, Параметры, Период);
...
Функция ДобавитьЗаданиеВнеОчереди(Знач Метод, Знач Параметры, Знач Период = Неопределено) Экспорт
	Задание = ПодготовитьМенеджераЗаписиЗадания(Метод, Параметры, Период);
...
Функция ВзятьЗаданиеВРаботу(Знач КлючЗадания, Знач КлючИсполнителя)
	...
	Если НаборЗаписей.Количество() = 1 Тогда
		...
		Если СостоянияДляВзятияВРаботу.Найти(Задание.Состояние) <> Неопределено Тогда
			...
			Задание.НачалоВыполнения = ТекущаяДатаСеанса();
...
Процедура ОтметитьЗавершениеЗадания(Знач КлючЗадания)
	...
	Если НаборЗаписей.Количество() = 1 Тогда
		...
		Задание.ЗавершениеВыполнения = ТекущаяДатаСеанса();
		НаборЗаписей.Записать();
...
Функция ПолучитьЗаданияИзОчереди(Знач КоличествоЗаданий)
	...
	|	мзЗадания.Период";
	ОднаМинута = 60;
	Запрос.УстановитьПараметр("ПериодМинутуНазад",  ТекущаяДатаСеанса() - ОднаМинута);
	РезультатЗапроса = Запрос.Выполнить();
...
Процедура ОсвободитьЗаданияБезАктивныхИсполнителей(Знач АктивныеИсполнители)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Задание.НачалоВыполнения + НастройкиМетодаОбработчика.ПаузаПередНовойПопыткой > ТекущаяДатаСеанса() Тогда
...
Функция ДождатьсяВыполнения(Знач КлючиЗаданий, Знач Таймаут = 5) Экспорт
	...
	Для каждого КлючЗадания Из КлючиЗаданий Цикл
		...
		Если Не ДождалисьВыполнения Тогда
			Прервать;
		КонецЕсли;
		ВремяВРаботе = ВремяВРаботе + (ТекущаяДатаСеанса() - НачалоОжидания);
	КонецЦикла;
...
Процедура СократитьИсториюВыполненияЗаданий(Знач ТребуемаяГлубинаИсторииВДнях) Экспорт
	МинимальныйПериодИстории = ТекущаяДатаСеанса() - ПолучитьОдинДень() * ТребуемаяГлубинаИсторииВДнях;
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	мзЗадания.Ключ
